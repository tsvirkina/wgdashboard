...
def register():
    ...
    try:
        ...
        name_check_query = g.cur.execute("SELECT * FROM users where username = '%s'" % (username)).fetchall()
        if len(name_check_query) > 0:
            return jsonify({"status": 400, "msg": "User with the same username alredy exist"})
        else:
            g.cur.execute("INSERT INTO users (username, password) VALUES ('%s', '%s')" % (username, password))
            add_user_check = g.cur.execute("SELECT id FROM users where username = '%s'" % (username)).fetchall()
            return jsonify({"status":200, "msg":f"User added, id: {add_user_check[0][0]}"})
    except:
